...
class TaskAwareResNet(nn.Module):
    def __init__(self, resnet: nn.Module, onehot_dim: int, init_std: float = 0.02):
        ...
        def _out_channels(layer):
            blk = layer[-1]
            return blk.conv3.out_channels if hasattr(blk, "conv3") else blk.conv2.out_channels
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.use_vae:
            ...
            self.register_buffer(
                "vae_encoder_pos_enc",
                create_sinusoidal_pos_embedding(num_input_token_encoder, config.dim_model).unsqueeze(
                    0
                ),
            )

        # ---------- Vision backbone  (with task-aware FiLM biases) ----------------
        if self.config.image_features:
            base_resnet = getattr(torchvision.models, config.vision_backbone)(
            ...
            self.backbone = TaskAwareResNet(
                base_resnet, config.onehot_action_dim
            )  # adds β after layers 2-3-4
            feat_C = base_resnet.fc.in_features  # channels of layer4

        # ---------- Transformer encoder & decoder (unchanged) ---------------------
        self.encoder = ACTEncoder(config)
        self.decoder = ACTDecoder(config)

        # ---------- Input projections into encoder --------------------------------
        if self.config.robot_state_feature:
    ...
    # ───────────────────────────────────────────────────────────────────────────────
    # COMPLETE forward  — VAE + multi-layer FiLM biases, with uniform (seq,B,D) dims
    # ───────────────────────────────────────────────────────────────────────────────
    def forward(
        ...
        # ==========================================================================
        # 1. LATENT via VAE encoder  (unchanged from before, but returns `latent`)
        # ==========================================================================
        ...
        # helper → (1,B,D) slice for 1-D positions
        def _pos(idx: int) -> Tensor:
            return (
                self.encoder_1d_feature_pos_embed.weight[idx]
                .expand(B, -1)                # (B,D)
                .unsqueeze(0)                 # (1,B,D)
            )

        # ==========================================================================
        # 2. BUILD token & pos lists  (each element is (seq,B,D))
        # ==========================================================================
        tokens: list[Tensor] = []
        poses: list[Tensor] = []

        # latent
        tokens.append(self.encoder_latent_input_proj(latent).unsqueeze(0))         # (1,B,D)
        poses.append(_pos(0))
        cursor = 1
        ...
        if self.use_onehot:
            ...
            tokens.append(self.encoder_onehot_action_proj(onehot).unsqueeze(0))
            poses.append(_pos(cursor))
        else:
            onehot = None  # still used by image backbone
        ...
        # ==========================================================================
        # 4.  CONCAT seq dimension  →  (seq,B,D)
        # ==========================================================================
        enc_tok = torch.cat(tokens, 0)
        enc_pos = torch.cat(poses, 0)
